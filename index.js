...
console.log(addArtist(artists,'Taylor', '1997-2022', 'web design', 'american', 'lorem ipsum'))
...
*/
function artistInfo(artists, artName){
  let artsInfo =[]
for(let i = 0; i< artists.length; i++){
  if(artists[i].name.includes(artName)){
    artsInfo.push(artists[i].bio)
  }
}
return artsInfo
}
...
function artistByCountry(artists, origin){
  let spanishArr =[]
for(let i = 0; i< artists.length; i++){
  if(artists[i].nationality.split(' ').includes(origin)){
    spanishArr.push(artists[i].name)
  }
}
return spanishArr
}
